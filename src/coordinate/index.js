...
class Coordinate {
  ...
  getAxis(d, children) {
    ...
    const toMap = function(key, d, children){
      const m = {};
      if (d.pickChildren && Array.isArray(children)) {
        d.data.map((k, i) => {
          const c = children[i];
          if (c && c.type === 'chart') {
            m[k] = toMap(k, c.coordinate.x, c.children)
          } else {
            m[k] = k
          }
        })
  ...
  scaleBand(d, range) {
    const domain = Coordinate.getDomain(d, this.children);
    const axis = this.getAxis(d, this.children);
    const scale = d3.scaleBand()
      .domain(domain.map((dd, i) => i))
      .range(range)
    ...
    return { scale, ticks: domain, axis };
  }
  ...
  idxToRoot(idx) {
    return Coordinate.getDomain(this.option.x, this.children.slice(0, idx)).length
  }
  ...
  bw(idx) {
    ...
    if (pickChildren) {
      const d = Coordinate.getDomain(this.option.x, this.children.slice(idx, idx + 1));
      r = d.length;
    }
